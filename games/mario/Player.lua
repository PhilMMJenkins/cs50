...
--Checks to see if we collide with flagpole
function Player:checkFlagOverlap()
    if self.map:flagpoles(self.map:tileAt(self.x - 1, self.y)) or self.map:flagpoles(self.map:tileAt(self.x + self.width, self.y)) then
        self.sounds['coin']:play()
